		  ...
		  //请求方法
		    ...
		    , {
		        ...
		        'click .top': function (e, value, row, index) { 
		        	jp.confirm('确认更改置顶状态吗？', function(){
                       	jp.loading();
                       	jp.get("${ctx}/meiguotong/comarticle/comArticle/top?id="+row.id, function(data){
                   	  		if(data.success){
                   	  			$('#comArticleTable').bootstrapTable('refresh');
                   	  			jp.success(data.msg);
                   	  		}else{
                ...
                formatter:  function operateFormatter(value, row, index) {
					...
					(row.isTop==1?'<a href="#" class="top" title="禁用"><i class="fa fa-edit btn btn-warning btn-xs">取消置顶</i> </a>':
					(row.isTop==2?'<a href="#"  class="top" title="正常"><i class="fa fa-edit btn btn-success btn-xs">置顶</i> </a>':'')),
    		        ].join('');
    		    }
            }
		     ]
